...
class CBSTTU_Planner:
    ...
    def compute_all_paths_and_conflicts(self, root, time_limit=60):
        ...
        for agent_id, agent_start in self.tu_problem.start_positions.items():
            agent_plan = self.planner.compute_agent_path(
                root.constraints, agent_id, agent_start, self.tu_problem.goal_positions[agent_id], root.conflict_table, self.tu_problem.subgoal_action_time,
                mbc=self.min_best_case, curr_time=self.curr_time, time_limit= time_limit)
    ...
    def generate_constraint_node(self, new_cons, best_node, time_limit):
        ...
        new_plan = self.planner.compute_agent_path(new_node.constraints, agent,
            ...
            best_node.conflict_table, self.tu_problem.subgoal_action_time,
